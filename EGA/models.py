...
class Subsession(BaseSubsession):
    ...
    def creating_session(self):
        for p in self.get_players():
            ...
            delta_r = Constants.range_delta
            stuck = Constants.stuck

            # create list of lottery indices
            # --------------------------------------------------------------------------------------------------------
            indices = [j for j in range(1, n + 1)]

            # create list of low and high outcomes and probs (matched by index)
            # --------------------------------------------------------------------------------------------------------
            outcomes_lo = [c(1) for j in range(0, n)]
            outcomes_hi = [c(Constants.sure_payoff + Constants.delta_hi * j) for j in range(0, n)]

            probs_lo = [0  +  Constants.probability_delta * j for j in range(0, n)]
            probs_hi = [100 - Constants.probability_delta * j for j in range(0, n)]

            # create a list of ambiguity ranges
            cd = sorted(stuck * [0 + Constants.probability_delta * j for j in range(0, int(n / stuck))])
            dd = [x + delta_r for x in cd]
            range_lo = ['-'.join(x) for x in zip([str(x) for x in cd], [str(x) for x in dd])]
            ed = sorted(stuck * [100 - Constants.probability_delta * j for j in range(0, int(n / stuck))])
            gd = [x - delta_r for x in ed]
            range_hi = ['-'.join(x) for x in zip( [str(x) for x in gd],[str(x) for x in ed])]
            ...
            # --------------------------------------------------------------------------------------------------------
            p.participant.vars['scl_lotteries_ega'] = list(
                zip(indices, outcomes_lo, outcomes_hi, probs_lo, probs_hi, range_lo, range_hi)
            )
            ...
            if Constants.random_order:
                random.shuffle(
                    p.participant.vars['scl_lotteries_ega']
                )
...
class Player(BasePlayer):
    ...
    p_lo = models.IntegerField()
    p_hi = models.IntegerField()
    p_range_lo = models.StringField()
    p_range_hi = models.StringField()
    # set payoffs
    # ----------------------------------------------------------------------------------------------------------------
    def set_payoffs(self):
        ...
        # ------------------------------------------------------------------------------------------------------------
        lottery_selected = [i for i in self.participant.vars['scl_lotteries_ega'] if i[0] == self.lottery_choice]
        ...
        self.p_lo = lottery_selected[3]
        self.p_hi = lottery_selected[4]
        self.p_range_lo = lottery_selected[5]
        self.p_range_hi = lottery_selected[6]
